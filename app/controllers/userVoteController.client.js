...
(function(){
    ...
    function displayNotifications(notificationData) {
        ...
        if (numberOfNotes > 0) {
            ...
            jsonNotes.map((note, index) => {
                if(index < 5){
                    var notification = "<li><a href='' class='myPoll' id='nI" + note.pollId + "'>" + note.message + index + "</a></li>";
    ...
    function displaySuggestions(userSuggestions){
        ...
        data.map((x) => $("#suggestions").append("<li pollId='poll" + x.pollId + "' class='noteButtons' id='id" + x._id + "'>" + x.message + "<span><button class='implementNote'>Implement</button></span><span><button class='deleteNote'>Delete</button></span></li>"));
        configNoteButtons();
